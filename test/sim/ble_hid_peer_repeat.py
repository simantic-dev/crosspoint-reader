# Scripted BLE HID-over-GATT page-turner that keeps pressing (ScriptedBlePeer).
#
# Identical to ble_hid_peer.py except that the right-arrow press/release pair
# repeats every REPEAT_DELAY_US of VIRTUAL time for as long as the host stays
# subscribed, instead of firing once on subscribe. One press is enough to prove
# the HID path works at all; a coexistence run needs presses arriving DURING a
# long section build, which is a window the single-shot peer cannot reach.
#
...
# One page turn: right-arrow press, then release.
PAGE_TURN_BURST = (bytes([0x01, 0x00, 0x00, KEY_RIGHT_ARROW, 0, 0, 0, 0, 0]),
...
# Gap between the press report and its release report.
KEYPRESS_DELAY_US = 100_000

# Virtual time between one page-turn press and the next. 8 s is slower than a
# person reading a page and much slower than the e-ink repaint, so a missed page
# turn is the firmware dropping the key rather than the peer outrunning it.
REPEAT_DELAY_US = 8_000_000
...
class Peer:
    ...
    def on_timer(self):
        ...
        if self.queue:
            ...
            # Burst drained: line up the next page turn.
            self.queue = list(PAGE_TURN_BURST)
            self.ctx.schedule_oneshot(REPEAT_DELAY_US)
