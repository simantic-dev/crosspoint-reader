...
#       size: 0x1000
#       numberOfInterrupts: 7
#       file: "<path>/xteink_board_helper.py"
#       0 -> gpio@3
#       1 -> saradc@0
#       ...
#       6 -> saradc@5
...
# Ladder-button lines (irq index = saradc index + 1, from the replx wiring).
BACK, CONFIRM, LEFT, RIGHT, UP, DOWN = 1, 2, 3, 4, 5, 6

PRESS_US = 400_000       # short press: ButtonNavigator's hold threshold is 500 ms


def _journey_steps():
    """The recorded scan journey (journey-ble-scan.txt), in virtual time:
    reader menu -> Go Home -> Settings -> Controls -> Bluetooth -> Scan ->
    select the first result. Delays are relative to the previous step."""
    seq = []
    def press(line, settle_us):
        seq.append((settle_us, line, True))
        seq.append((PRESS_US, line, False))
    press(CONFIRM, 9_400_000)          # reader menu (~12 s virtual)
    for _ in range(10):
        press(DOWN, 1_500_000)         # down to "Go Home"
    press(CONFIRM, 2_500_000)          # Go Home
    press(DOWN, 6_500_000)             # home painted; move toward Settings
    for _ in range(3):
        press(DOWN, 1_500_000)
    press(CONFIRM, 2_500_000)          # Settings (after 6 s home paint... margin below)
    press(CONFIRM, 5_000_000)          # into category
    press(CONFIRM, 2_500_000)          # Controls
    press(UP, 3_000_000)               # Bluetooth row
    press(CONFIRM, 3_000_000)          # open Bluetooth screen
    press(CONFIRM, 6_000_000)          # toggle Bluetooth ON
    press(DOWN, 8_000_000)             # stack up + repaint; move to Scan row
    press(CONFIRM, 2_000_000)          # start scan
    press(CONFIRM, 30_000_000)         # select the first scan result
    return seq
