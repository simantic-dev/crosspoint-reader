...
# real Free 2 emits. Repeated so a click also lands after the UI has been driven
# somewhere the key can act; this is bench stimulus (when a human presses the
# remote), not a modelled quantity.
...
class Peer:
    ...
    def on_disconnect(self, reason):
        # Clear the subscription: without this the peer keeps "notifying" into a link
        # that no longer exists, and the log reads as if keys were still delivered.
        self.subscribed = False
        self.queue = []
